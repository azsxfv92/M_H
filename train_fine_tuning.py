...
def train(hyp, opt, device, callbacks):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # ================================================================================================
            # [목적] Pruned 모델의 희소성(sparsity) 유지 및 학습 진행 상황 가시성 개선
            #
            # 1. Gradient Masking: Pruning으로 0이 된 가중치가 재학습되지 않도록 해당 위치의
            #    gradient를 0으로 강제 설정하여 희소성 패턴을 유지
            # 2. Custom Progress Bar: 학습 진행률을 퍼센트와 이미지 수로 명확하게 표시하여
            #    학습 상태를 직관적으로 모니터링
            # ================================================================================================
            #KMH_TEST_START
            ...
            if ni - last_opt_step >= accumulate:
                ...
                # Pruned 가중치의 희소성 유지를 위한 gradient masking
                with torch.no_grad():
                    for name, param in model.named_parameters():
                        # 훈련 가능한 파라미터(기울기가 계산된 파라미터)만 처리
                        if param.grad is not None:
                            # 가중치 값이 정확히 0인 위치를 마스크로 저장
                            zero_mask = (param.data == 0)

                            # 마스크 위치의 기울기를 0으로 강제 설정
                            # 이를 통해 0이었던 가중치는 0을 유지하게 됩니다.
                            param.grad.data[zero_mask] = 0.0
            ...
            # [커스텀 Progress Bar - 가독성 향상]
            if RANK in {-1, 0}:
                mloss = (mloss * i + loss_items) / (i + 1)  # update mean losses
                mem = f"{torch.cuda.memory_reserved() / 1e9 if torch.cuda.is_available() else 0:.3g}G"  # GPU memory (GB)

                # Progress bar 정보 계산
                progress = (i + 1) / nb * 100   # 현재 epoch 진행률 (%)
                current_images = (i + 1) * imgs.shape[0]  # 현재까지 처리한 이미지 수
                total_images = nb * imgs.shape[0]  # 전체 이미지 수

                # 사용자 친화적인 progress bar 표시
                pbar.set_description(
                    f"Epoch [{epoch+1}/{epochs}] "
                    f"{progress:5.1f}% ({current_images}/{total_images} imgs) | "
                    f"Loss: {mloss.sum():.4f} | "
                    f"Mem: {mem}"
                )
            #KMH_TEST_END
        ...
        if RANK in {-1, 0}:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ckpt = {
                    ...
                    "model": deepcopy(de_parallel(model)),  # fp32로 저장 (.half() 제거)
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = validate.run(
                        ...
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
                        model=attempt_load(f, device),  # fp32로 검증 (.half() 제거)
